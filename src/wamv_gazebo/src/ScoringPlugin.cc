...
class ScoringPlugin::Implementation
{
  ...
  public: std::string taskInfoTopic = "/wamv_gazebo/task/info";

  /// \brief Topic name to release the vehicle if locked to the world.
  public: std::string releaseTopic = "/wamv_gazebo/release";
    ...
    sim::EventManager &_eventMgr)
{
  ...
  this->dataPtr->node.Subscribe("/wamv_gazebo/contacts",
...
GZ_ADD_PLUGIN_ALIAS(wamv_gazebo::ScoringPlugin,
                    "wamv_gazebo::ScoringPlugin")
