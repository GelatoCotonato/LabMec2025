...
import wamv_gazebo.payload_bridges
import pathlib
import re
import shutil
import yaml
...
class Model:
    ...
    def bridges(self, world_name):
        ...
        bridges = [
            # pose
            wamv_gazebo.bridges.pose(self.model_name),
            # pose static
            wamv_gazebo.bridges.pose_static(self.model_name),
            # joint states
            wamv_gazebo.bridges.joint_states(world_name, self.model_name),
            # comms tx
        ...
        if self.is_UAV():
            bridges.extend([
                # Magnetometer
                wamv_gazebo.bridges.magnetometer(world_name, self.model_name),
                # Air Pressure
                wamv_gazebo.bridges.air_pressure(world_name, self.model_name),
            ])
            bridges.extend([
                # twist
                wamv_gazebo.bridges.cmd_vel(self.model_name)
            ])
        elif self.is_USV():
            pass
    ...
    def payload_bridges_impl(self, world_name, payloads):
        ...
        for sensor_name, value in payloads.items():
            ...
            bridges.extend(
                wamv_gazebo.payload_bridges.payload_bridges(
